...
#if defined(CLDERA_PROFILING)
                  if (icall < 2) then ! profile climate calculation and diags
                     if (masterproc) then
                       write(iulog,*)'GH setting diagnostic variable data icall=', icall, ' with name=', diag(icall)
                     endif
       ...
       if (dolw) then
          ...
          do icall = N_DIAG, 0, -1
              ...
              if (active_calls(icall)) then
                  ...
                  if (icall < 2) then ! profile climate calculation and diags
                     if (masterproc) then
                       write(iulog,*)'GH setting diagnostic flux data icall=', icall, ' with name=', diag(icall)
                     endif
